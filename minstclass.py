# -*- coding: utf-8 -*-
"""
Created on Sat May 19 20:56:54 2018
...
class Data:
    ...
    def predict(self):
        ...
        return
    def predict_online(self, img):
        #lena = mpimg.imread(img)
        im = Image.open(img)
        im_array = np.array(im)
        im_array=im_array.reshape((1,784))
        hidden_layer1 = np.maximum(0, np.matmul(im_array, self.W1) + self.b1)
...
if __name__ == '__main__':            
    ...
    data.train()
    data.predict()
    #pickle.dump( data.loss_list, open( "gradient_data", "wb" ), False )
    pickle.dump( data, open( "data", "wb" ), False )
